...
class CustomXMPP(object):
    ...
    def __init__(self, hass, jid, password, recipient_me, recipient_group):
        self._hass = hass
        self._jid = jid
        self._password = password
        self._recipient_me = recipient_me
        self._recipient_group = recipient_group
        self._XmppNotificationService = XmppNotificationService(
            sender=self._jid,
            resource=None,
            password=self._password,
            recipient=self._recipient_me,
            tls=None,
            verify=None,
            room=None,
            hass=self._hass
    ...
    def send_to_me(self, message=""):
        ...
        self._XmppNotificationService.async_send_message(message)
...
async def async_send_message(
    ...
    class SendNotificationBot(slixmpp.ClientXMPP):
        ...
        def send_text_message(self):
            ...
            try:
                _LOGGER.info('send_text_message')
                if room:
                    _LOGGER.info("Joining room %s", room)
                    self.plugin["xep_0045"].join_muc(room, sender, wait=True)
                    self.send_message(mto=room, mbody=message, mtype="groupchat")
                else:
                    _LOGGER.info("Sending message to %s", recipient)
